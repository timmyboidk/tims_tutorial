...
export const frontendM1M2: Lesson[] = [
    {
        ...
        startingCode: '',
        instructions: `# 配置 SaaS 项目的 React 外壳\n\n## 业务上下文\n在开始编写我们“短视频 SaaS 看板”的任何业务代码前，我们需要一个坚实的基础。本项目将采用 Vite + React + Tailwind CSS。\n\n## 学习目标\n- 理解 **Vite** 的代理机制如何解决前后端分离导致的跨域问题。\n- 掌握 Tailwind CSS 的原子化配置。\n\n## ⚠️ 为什么要这么做？\n我们的后端运行在 \`localhost:8080\`，如果我们前端直接 \`fetch('localhost:8080/api')\`，浏览器会报错 CORS（跨域）。通过在 Vite 配置中加入代理，我们可以让请求伪装成同源。`,
    ...
    },
    {
        ...
        instructions: `# 类型严格的登录表单\n\n## 业务上下文\nSaaS 系统的第一道门槛就是登录。当用户输入账号密码后，我们会调用 API，后端将返回一个 JWT Token 和 User 信息。如果没有准确描述返回信息长什么样，随着业务迭代你的代码极易出现 \`undefined\` 错误。\n\n## 学习目标\n- 学会定义网络请求的 Playload 与 Response 接口（Interfaces）。\n- 使用 \`React.FormEvent\` 定义事件类型。\n- 使用泛型强约束状态钩子 \`useState<T>\`。`,
    ...
    },
    {
        ...
        startingCode: '',
        instructions: `# 启发式实战：SaaS 级强类型 UI Button\n\n## 业务上下文\n在 SaaS 看板中，你到处需要按钮：登录、删除视频、发酵数据。我们不要每次都手写 Tailwind 样式。这里要求你编写一个基于泛型和字面量联合类型的 Button，并且用 \`Record\` 限定样式，使得团队里的其他人拿去用时如果写错了 \`variant\` 就会被 TypeScript 报错。`,
    ...
    },
    {
        ...
        startingCode: '',
        instructions: `# 优化短视频 Feed 流卡片\n\n## 业务上下文\n当后台数据传回数千条视频时，你在浏览器中要渲染一个 Feed 流。如果父组件的状态（比如顶部全局过滤器的切换）发生了改变，整个庞大的 Feed 流会被 React 默认全军覆没地**重新渲染**。我们要保护好这几十万个 DOM 节点。\n\n## 学习目标\n- 使用 \`React.memo\`：当 props 没有变化时跳过重绘。\n- 结合业务逻辑定义 \`VideoCard\` 结构。`,
    ...
    },
    {
        ...
        startingCode: '',
        instructions: `# 构建无限滚动 Custom Hook\n\n## 业务上下文\n所有短视频架构（如抖音/TikTok）都采用向下划无线加载的交互。这依赖于浏览器的 IntersectionObserver API（当底部的占位元素滚进屏幕就发请求）。为了在项目中反复复用，我们将其提取为一个 \`useIntersectionObserver\` Hook。\n\n## 学习目标\n- 设计高级可复用 React Hook。\n- 使用 \`useRef\` 保留回调引用以抵御闭包陷阱。`,
    ...
    },
    {
        ...
        startingCode: '',
        instructions: `# 使用 Web Worker 卸载繁重计算\n\n## 业务上下文\n当数据看板里缓存了数千条短视频数据时，如果用户点击了“按热度排序”，直接在主线程中进行 \`Array.prototype.sort()\` 会导致 React 卡死网页，动画掉帧。我们将计算从主航道剥离，抛入 Web Worker 行星发动机中异步计算。\n\n## 学习目标\n- \`Blob(..)\` 动态创建独立线程环境。`,
    ...
    },
    {
        ...
        startingCode: '',
        instructions: `# 启发式实战：聚合功能，组装视频流界面\n\n## 业务上下文\n在这节课里，你需要像拼装乐高一样，把我们做好的 \`useIntersectionObserver\`、\`VideoCard\` 给串起来。一旦到底部自动获取 \`/api/feed/page=...\`，完成我们的核心前端框架展现。这是一个极其标准的业务闭环。`,
